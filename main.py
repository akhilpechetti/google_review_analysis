...
df = dataset_creation(None)
if cat == 'View Reviews':
    empty_df = pd.DataFrame()
    sel = st.sidebar.selectbox('Select One', [
                               'View reviews based on Rating given by reviewer', 'View all Reviews irrespective of Rating'])
    # if cat=='Datasets':
    if sel == 'View reviews based on Rating given by reviewer':
        ...
        st.download_button(label='Download CSV', data=empty_df.to_csv(
        ), mime='text/csv', file_name='Reviews based on rating.csv')
    if sel == 'View all Reviews irrespective of Rating':
        st.dataframe(df)
        st.download_button(label='Download CSV', data=df.to_csv(),
                           mime='text/csv', file_name='Reviews.csv')
if cat == 'Sentiment analysis':
    ...
    if sen_cat == 'Dataset with Sentiment score and Subjectivity':
        ...
        st.download_button(label='Download CSV', data=scores_df.to_csv(
        ), mime='text/csv', file_name='Reviews with Sentiment score.csv')
